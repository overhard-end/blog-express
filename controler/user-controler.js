...
class UserController {
  ...
  async changeUser(req, res, next) {
    try {
      ...
      const { id, dataForChange } = bodyAsJson;
      const data = JSON.parse(dataForChange);
      const file = req?.files?.file;

      const ChangedUserData = await userService.changeUserData(id, data, file);
      return res.json(ChangedUserData);
    } catch (e) {
      next(e);
    }
  }
